...
def process_silver_layer(connection_uri: str) -> pl.DataFrame:
    ...
    SELECT
        ...
        raw_content->>'License Type' as licensure,
        raw_content->>'Approval Date' as approval_date,
        raw_content->>'Orphan Exclusivity Expiration' as orphan_exclusivity_date,
    ...
    # Filter out any repeated header rows and empty rows
    df = df.filter(
        pl.col("bla_number").is_not_null() & 
        (pl.col("bla_number").str.strip_chars() != "") & 
        (pl.col("bla_number") != "BLA Number")
    )
    ...
    # Composite key generation for true primary key uniqueness
    valid_df = valid_df.with_columns(
        composite_key=pl.concat_str(
            [
                pl.col("bla_number").fill_null(""),
                pl.col("strength").fill_null(""),
                pl.col("product_presentation").fill_null("")
            ], 
            separator="|"
        )
    )
    ...
    return valid_df.with_columns(
        source_id=pl.col("bla_number"), 
        coreason_id=get_coreason_id_expr("composite_key")
    ).drop("composite_key")
